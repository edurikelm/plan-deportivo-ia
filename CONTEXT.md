...
- `suggestRepsForExercise(records, exerciseName): number` — helper puro en `src/lib/calculator/suggest-reps.ts`. Sugiere un default de reps para el form de Guardar con etiqueta, basado en el set más reciente del mismo ejercicio: retorna `1` si no hay registros previos o el más reciente fue de 1 rep; `Math.max(5, lastReps)` si el más reciente fue multi-rep. Matchea case-insensitive e ignora registros con `reps: null` o `exercise: null`. El default se aplica **una sola vez** al montar el form; las ediciones manuales del coach no se sobreescriben.
...
- Abre un **form inline** (no modal, no drawer) con input `Ejercicio` (obligatorio) + datalist de `getUniqueExercises()`, e input `Repeticiones` (obligatorio, entero ≥ 1, default 1, default sube a 5 si el último set del mismo ejercicio fue > 1). Checkbox opcional `Marcar como 1RM` debajo.
- Submit deshabilitado si `exercise.trim() === ""` o `reps < 1`. Sin carga real (`discs.length === 0 && barKg === 20`), el botón Guardar está disabled.
...
- **Estado compartido de la calculadora** — único objeto `{ barKg, discs }` que la pestaña Manual lee y escribe (y, antes de su desactivación en 0040, también la Foto tab vía el modal de aceptación). La nueva pestaña **Por objetivo** (0048) **no** participa de este estado: tiene su propio selector de barra local y no muta `{ barKg, discs }` en ningún momento. Es una herramienta de consulta stateless.
...
- **Persistencia de la calculadora** — el estado `{ barKg, discs }` se auto-guarda en `localStorage` key `pd:calculator-state` con debounce en cada cambio. Al cerrar y volver, la calculadora abre con la última carga. Sin `beforeunload` guard (nada se pierde nunca). Sin mini-historial. Botón ghost `Limpiar` arriba a la derecha resetea a defaults (`{ barKg: 20, discs: [] }`) con `window.confirm`. La pestaña **Por objetivo** no persiste nada: cada visita arranca con `barKg = 20` y un input vacío.

### Resolver (pestaña "Por objetivo" en `/tools/weight-calculator`)

Feature stateless introducida en 0048 que calcula la carga (barra + discos por lado) a partir de un peso objetivo. Es la operación **inversa** de la pestaña Manual: en Manual el Entrenador compone la carga y ve el total; en Por objetivo tipea un peso total y obtiene los discos.

- **Resolver** — acción de calcular la carga desde un peso objetivo. Implementado como pure function en `src/lib/calculator/resolver.ts`. Stateless; no comparte state con la pestaña Manual ni escribe en `pd:calculator-state` ni en `pd:calculator-records`.
- **Peso objetivo** — input del resolver. El text field acepta `135`, `135 lb`, `135lb`, `135 lbs`, `60 kg`, `60kg`, decimales como `62.5`. La unidad se infiere del sufijo; si no hay sufijo, usa el toggle de display unit (kg/lb) arriba a la derecha.
- **Resultado exacto** — combinación de discos (uno por lado) que iguala el peso objetivo dentro del inventario por defecto. Se muestra con el badge `Resultado exacto` y un borde signal.
- **Vecino más cercano** — cuando no hay exacto: dos cards lado a lado, `Por arriba` (sum mínima ≥ target) y `Por abajo` (sum máxima ≤ target), cada una con su desglose y un badge `±X kg / ±Y lb` indicando la diferencia contra el objetivo. El coach decide cuál cargar.
- **Inventario por defecto** — set hardcodeado de tamaños disponibles para el resolver: kg `[25, 20, 15, 10, 5, 2.5, 1.25]`, lb `[45, 35, 25, 10, 5, 2.5]`. Configurable por box queda fuera de scope; se cubre en una V2 cuando se agregue la sección "Placas del box" en `/settings`.
- **Greedy descendente** — algoritmo base: del disco más grande al más chico, poner los que entren en el remanente. No óptimo en conteo de discos; sí simple, determinista y suficiente para un box real. Cuando el greedy falla (no llega exacto), la búsqueda de vecino más cercano reusa el mismo inventario con bounded DFS: el "arriba" es la sum mínima ≥ perSide, el "abajo" la sum máxima ≤ perSide. Bounded por `ceil(perSideKg / smallestDisc) + 1` que para targets típicos corre en < 1 ms.
- **Output del resolver** — la pestaña muestra breakdown (`20kg + (25kg + 10kg)×2`), totales en kg y lb, y la `BarVisualization` reusada del Manual (ancho de cada disco proporcional al peso en kg). Sin botón "Copiar" ni "Aplicar a Manual": el resultado es de lectura, no de mutación. Si el Entrenador quiere persistir la carga, va a la pestaña Manual y la compone manualmente.

Por construcción, el resolver **no** muta el `CalculatorState` de Manual. La salida es solo lectura y visualización. El test suite cubre: match exacto, vecinos (arriba/abajo), target < bar (null), target = bar (bar only), unidades mixtas (target en lb con bar en kg), inventario custom, target entre dos tamaños de disco, target grande (200 kg), y el set default.
- **SavedWeightRecord** — unidad durable de un cálculo de peso persistido en `pd:calculator-records`. Snapshot de `{ barKg, discs, totalKg, totalLb, breakdownLine, exercise, source, reps?, isOneRepMax? }`. Se crea por dos caminos: (a) **Guardar con etiqueta** explícito (`exercise` y `reps` obligatorios, `source: "manual"`), (b) **Foto attribution** cuando el coach acepta una foto (`exercise: null`, `reps: null`, `source: "foto"`, persistido inmediatamente). Los registros foto quedan fuera del análisis de ejercicio (no tienen `exercise` para agrupar).
- **Guardar con etiqueta** — acción explícita del coach. Abre un form inline con campos `Ejercicio` (obligatorio), `Repeticiones` (obligatorio, entero ≥ 1, default sugerido por `suggestRepsForExercise` a partir del último set del mismo ejercicio), y checkbox `Marcar como 1RM` (opcional, mapea a `isOneRepMax`). Persiste un `SavedWeightRecord` con `source: "manual"`.
...
- **Página de historial** — `/tools/weight-calculator/history` (post-refactor: lista de **ejercicios** agrupados a partir de `pd:calculator-records`). Cada fila es un ejercicio único; click → vista de análisis.
- **Foto tab UX** — un solo botón "Elegir foto" (`<input type="file" accept="image/*">` sin `capture`, mobile chooser nativo muestra cámara/galería/archivos). Cuatro estados: (a) vacío con copy + botón + privacy disclosure inline, (b) foto cargada con thumbnail + botones `Elegir otra` / `Analizar`, (c) analizando con status strip signal-fill + label `Analizando…` (sin spinner en el botón), (d) preview del desglose. Constraints client-side: max 5MB, JPEG/PNG/WebP, min 200×200px. **Estado actual: desactivada**. La tab sigue visible en el selector Manual/Foto de la calculadora pero su contenido está bloqueado: al seleccionarla, se muestra un placeholder `Función desactivada` (sin foto upload, sin analizar, sin preview). El código del Foto tab y el endpoint `/api/calculate-weight` se preservan intactos para reactivación futura. **Out of scope del análisis de ejercicio**: los registros foto históricos (`exercise: null`, `reps: null`) no entran al análisis.
...
## Análisis de Ejercicio (vista por ejercicio)

Página de drill-down por ejercicio. Toma como entrada **un único** `exercise` (string) seleccionado desde el catálogo refactorizado y deriva tres vistas: charts, tabla de RM, e historial del ejercicio.

- **Entry point desde `/classes`** — `/classes` (catálogo) suma una nueva card `Ejercicios guardados` en la sección `HERRAMIENTAS`, debajo de la card actual `Calculadora de Pesos`. El entry de la card apunta a `/tools/weight-calculator/history` (que pasa a ser la **lista de ejercicios**, no la lista plana de registros).
- **`aggregateByExercise(records): ExerciseSummary[]`** — helper puro en `src/lib/calculator/aggregate.ts`. Agrupa los `SavedWeightRecord` con `exercise !== null` por nombre case-insensitive, preserva la capitalización del registro más reciente como nombre canónico, computa `recordCount`, `lastRecordAt`, `lastTotalKg`, `bestTotalKg` (max entre `reps === 1`), y `estimatedOneRmKg` (vía `aggregateExerciseOneRepMax`). Devuelve los summaries ordenados por `lastRecordAt` desc. Excluye explícitamente records con `exercise === null` (auto-log, foto).
- **`getRecordsForExercise(records, exerciseName): SavedWeightRecord[]`** — helper puro, retorna los registros de UN ejercicio (match case-insensitive), ordenados por `createdAt` desc. Retorna `[]` para un ejercicio sin registros (en vez de explotar). Es la primitiva que la vista de análisis (0039) usa para cargar el snapshot del ejercicio seleccionado.
- **`/history` refactorizado** — la ruta actual que lista todos los `SavedWeightRecord` se refactoriza para listar los **ejercicios únicos** derivados. Cada fila muestra: nombre del ejercicio, cantidad de registros, último registro (`createdAt` + `totalKg`), mejor registro (`max totalKg` con `reps === 1` si existe, sino el `e1RM` estimado). Click en una fila → `/tools/weight-calculator/exercise/[name]` (la vista de análisis).
- **Vista de análisis de ejercicio** — ruta `/tools/weight-calculator/exercise/[name]`. Deep-linkable. Codifica el nombre del ejercicio en el segmento `[name]`; la búsqueda de registros es case-insensitive (compara contra `normalizeExerciseName`-style lowercase) pero preserva la capitalización del primer registro encontrado para mostrar.
- **Ejercicio seleccionado** — agrupa todos los `SavedWeightRecord` con `exercise` que matchea (case-insensitive). Excluye `auto-log` y registros con `exercise === null`.
- **1RM estimado** — `1RM_ejercicio = max(1RM_formula_por_registro)` sobre todos los registros del ejercicio con `reps` no-null. Fórmula default **Epley**: `1RM = totalKg × (1 + reps / 30)`. Para `reps === 1`, se usa `totalKg` directo (Epley sobreestima ~3%). Si hay registros con `isOneRepMax === true`, se incluye también `1RM_flag = totalKg` del marcado de mayor `totalKg`, y se toma `max(1RM_formula, 1RM_flag)` para el análisis.
- **Marcar 1RM** — acción opcional. Togglea `isOneRepMax` sobre un registro. Persiste inmediato en `pd:calculator-records`. Sirve para casos donde la fórmula no aplica (1RM testeado en competencia, o un set con `reps === 1` y peso max).
- **Charts** — tres charts en una grilla 3-col en desktop, stack vertical en mobile. Renderizados con **Recharts** (`LineChart` + `BarChart` + `ResponsiveContainer`). Tres series:
  1. **Progresión de peso total** — `LineChart` `totalKg` (eje Y) vs `createdAt` (eje X, serie temporal).
  2. **Volumen por sesión** — `BarChart` con `totalKg × reps` (eje Y) por registro (asumimos "1 set" porque el modelo actual no persiste `sets`; si se agrega, este chart se vuelve `sets × reps × totalKg`).
  3. **e1RM rolling** — `LineChart` con ventana rolling (default 3 últimos registros) que muestra el `1RM estimado` por registro, dando tendencia.
- **Tabla de RM** — tabla de Prilepin clásica (1–12 reps × % de 1RM) con columnas: `Reps`, `% 1RM`, `Peso (kg)`, `Peso (lb)`. Las columnas kg/lb se derivan del 1RM activo (no son editables). Sticky en desktop (sidebar 18rem), full-width stack en mobile.
- **Historial del ejercicio** — lista de los `SavedWeightRecord` del ejercicio, ordenados por `createdAt` desc, con `Cargar` (igual que la página de historial general) y el toggle `Marcar 1RM` por fila.
